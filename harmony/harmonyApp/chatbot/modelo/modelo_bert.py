...
def definir_pregunta(pregunta):
    ...
    for key, value in json_data.items():
        ...
        doc2 = nlpSpacy(' '.join(value).lower())
        similarity_scores[key] = doc1.similarity(doc2)
...
def respuesta_modelo_bert_contexto(pregunta):
    ...
    with open(ruta, 'r') as f:
        data = json.load(f)
        contexto = data[tipo_pregunta]  # Extraer el campo 'titanic' de los datos JSON
    if type(contexto) == list:
        contexto = ", ".join(contexto)

    print("contexto", contexto, type(contexto))
